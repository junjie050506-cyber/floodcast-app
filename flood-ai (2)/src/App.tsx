...
export default function App() {
      ...
      try {
        ...
        for (const c of newCitizensList) {
          if (c.status === 'EVACUATED' || c.status === 'SAFE') {
            ...
            if (activeSos) {
              await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'sos_requests', activeSos.id), {
                status: 'RESOLVED',
  ...
  const [showAllShelters, setShowAllShelters] = useState(true);
    ...
    if (user && db) {
      ...
      for (const req of userActiveRequests) {
        if (!req.id.startsWith('local-')) {
          ...
          setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'sos_requests', req.id), {
            status: 'RESOLVED',
            resolvedAt: serverTimestamp()
  ...
  const handleDispatchRescue = async (sosId) => {
    ...
    if (db) {
      ...
      const targetReq = (sosRequests as any[]).find(s => s.id === sosId);
      try {
        const payload: any = { status: 'DISPATCHED', dispatchedAt: serverTimestamp() };
        if (targetReq) {
          payload.zone = targetReq.zone || '';
          payload.lat = targetReq.lat || 0;
          payload.lon = targetReq.lon || 0;
          payload.userId = targetReq.userId || 'guest_resident';
          payload.name = targetReq.name || '';
          payload.phone = targetReq.phone || '';
          payload.message = targetReq.message || '';
          if (targetReq.timestamp && typeof targetReq.timestamp.toMillis === 'function') {
            payload.timestamp = new Date(targetReq.timestamp.toMillis());
          }
        }
        await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'sos_requests', sosId), payload, { merge: true });
      } catch (err) {
  ...
  const handleAcceptRescue = async (sosId) => {
    ...
    if (db) {
      ...
      if (acceptedRequest) {
        payload.zone = acceptedRequest.zone || '';
        payload.lat = acceptedRequest.lat || 0;
        payload.lon = acceptedRequest.lon || 0;
        payload.userId = acceptedRequest.userId || 'guest_resident';
        payload.name = acceptedRequest.name || '';
        payload.phone = acceptedRequest.phone || '';
        payload.message = acceptedRequest.message || '';
        if (acceptedRequest.timestamp && typeof acceptedRequest.timestamp.toMillis === 'function') {
          payload.timestamp = new Date(acceptedRequest.timestamp.toMillis());
        }
      }
      setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'sos_requests', sosId), payload, { merge: true }).catch(err => {
  ...
  const handleUpdateMissionStatus = async (status) => {
    ...
    if (db) {
      ...
      setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'sos_requests', (myActiveSos as any).id), {
        status: status,
  ...
  const toggleShelterCapacity = async (shelterId, currentIsFull) => {
    const isFullBool = !!currentIsFull;
    const nextIsFull = !isFullBool;
    
    // Optimistic UI state update so buttons function instantly
    setShelterStatus(prev => ({
      ...prev,
      [shelterId]: {
        ...(prev[shelterId] || {}),
        isFull: nextIsFull
    ...
    if (db) {
      try {
        await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'shelters', shelterId), { isFull: nextIsFull }, { merge: true });
        showToast(`Shelter marked as ${nextIsFull ? 'FULL' : 'OPEN'}`, 'success'); 
        addLog(`[ADMIN] Shelter capacity status overridden for ${shelterId}.`);
      } catch (err) {
        console.warn("Firestore error in toggleShelterCapacity, using local optimistic state:", err);
      }
    } else {
      showToast(`Shelter marked as ${nextIsFull ? 'FULL' : 'OPEN'} (Offline Mode)`, 'success');
      addLog(`[ADMIN] Shelter capacity status overridden for ${shelterId} (Offline Mode).`);
    }
  };
  ...
  const getShelterOccupancy = (shelter) => {
    ...
    if (statusObj?.isFull) {
      ...
      if (statusObj?.capacity !== undefined) {
        cap = statusObj.capacity;
        pct = (cap / maxCap) * 100;
      }
      if (statusObj?.isFull === false) {
        if (pct >= 100) {
          pct = 95;
          cap = Math.min(cap, Math.floor(maxCap * 0.95));
        }
      }
    }
